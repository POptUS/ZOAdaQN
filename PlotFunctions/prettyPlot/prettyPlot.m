...
% Raghu: Adding the code where markers are specified at specific locations
% In Options, include markerindices
% markerIndices - n by m array consisting of marker locations for each
% line. 
if nargin < 3
    options = [];
end
...
[legendStr,plotTitle,plotXlabel,plotYlabel,lineWidth,colors,lineStyles,...
    ...
    useLines,fillFace,errors,errorStyle,errorColors,errorFill,labelLines,labelLoc,labelRotate,markerIndices] = ...
    myProcessOptions(options,'legend',[],'title',[],'xlabel',[],'ylabel',[],...
    'lineWidth',3,'colors',[],'lineStyles',[],...
    'markerSize',10,'markers',[],'markerSpacing',[],...
    'xlimits',[],'ylimits',[],...
    'logScale',0,'legendLoc','Best','useLines',1,'fillFace',0,...
...
if isempty(markerSpacing)
    for i = 1:nLines
        ...
        if ~isempty(markerIndices) && ~isempty(markers{i})
            set(h(i),'Marker',markers{i},'MarkerIndices',markerIndices{i});
        end
    end
else
    for i = 1:nLines
...
if ~isempty(plotTitle)
    ...
    set(hTitle,'FontName','Arial','FontSize',20,'FontWeight','normal');%, 'Interpreter', 'latex');
end

if ~isempty(plotXlabel) || ~isempty(plotYlabel)
    h1 = xlabel(plotXlabel);
    h2 = ylabel(plotYlabel);
    set([h1 h2],'FontName','Arial','FontSize',20,'FontWeight','normal', 'Interpreter', 'latex');
end
    ...
    if labelLines % Try to label the lines
        for i = 1:nLines
            ...
            hText(i) = text(x{i}(m),y{i}(m),...
                ...
                'FontSize',24, 'Interpreter', 'latex');%,'FontWeight','bold');%, 'Intepreter', 'latex');
        ...
        set(h,'FontSize',20,'FontWeight','normal','FontName','Arial','Interpreter', 'latex');
        set(h,'Location','Best');
        set(h,'Location',legendLoc);
    end
end
vals=get(gcf, 'PaperPosition');
set(gcf, 'PaperPositionMode', 'auto', 'PaperSize',[vals(3) vals(4)]);
