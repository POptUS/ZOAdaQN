...
%     resultString = sprintf('%s/%s_%s_%e_%s_%s_(%s_%f_%e_%e_%e_%.2f_%.2f_%.2f_%.2e_%.2e_%e_%s_%s_%i_%s)_(%d_%d_%d_%e_%s_%s)_(%d_%s_%.2f_%s_%s_%d_%s_%s_%f_%i_%f_%i_%i)_%d',...
...
%     Options.L0, Options.zeta, Options.eta, Options.minL, Options.maxL,Options.step_tolr,Options.Frequent_check,Options.Frequent_rule,Options.Const_Step_Check,Options.step_extra,... 
%     Options.MaxIterations,Options.MaxEpochs,Options.MaxStorage, Options.epsilon,Options.StopTest,Options.Stop_extra,...
%     Options.S0, Options.SamplingTest,Options.theta,Options.Variance, Options.Vraiance_orthogonal, Options.nu, Options.Sample_decrease,Options.Sample_extra,Options.GeometricRate,...
%     Options.gamma,Options.Backup_norfactor,Options.Backup_samplefrequency,Options.Backup_ravgfrequency,...
%     i);
    ...
    if exist(resultFile_short,'file')
        ...
        %resultString_short = sprintf('%s/%s_%s_%.2e_%i_%.2f_%s_%s_%d',resultDir_short,data,Options.Method,Options.lambda,log2(Options.alpha0),Options.theta,Options.DFOMethod, Options.DFODirs,i);
        %resultFile_short=strcat(resultString_short,'.mat');
        %copyfile (resultFile, resultFile_short)
    %end
    %else
    %diary(resultString)
    ...
    save(resultFile_short,'w','batchsizes','Evals','funvals','norm2grad','norminfgrad','Sample_Obj','Sample_Grad'...
     ,'Stepsize','Descent', 'time','testfunvals','corrclass', 'Options','lsevals', 'iterations');
