...
fprintf('%15s %15s %15s %15s %15s %15s %15s %15s %15s\n',...
    'Iterations','Eff_passes', 'batch_size', 'Sample_Obj', 'Full_Obj','||Sample_grad||_2','||grad||_2', '||grad||_inf','alpha');
...
while NfunEvals < Options.MaxEpochs*ndatapnts
    ...
    tic;
    iter=iter+1;
    if iter==1 ||  (NfunEvals - count_stor_eval > stor_inter*ndatapnts) || iter < init_stor % included the storage such for initial iterations
        str_flag=1;
        count_stor_eval=NfunEvals;
    else
        str_flag=0;
    ...
    switch LineSearch
        case 'Constant'
            ...
            NLineEvals=0;
        case 'Armijo'
            [f_new,g_new, alphak, NLineEvals]= Armijo(Obj,inputvals,batch,f_s,d,g_s,w,Options.c, Options.rho, Options.alpha0);
            w = w + alphak*d;
    end
    alphak_old=alphak;
    if str_flag
    ...
    if str_flag
        [ftest, correct]=Obj.test(inputvals,w);
        fprintf('%15i %12.6f %15i %15.5e %15.5e %15.5e %15.5e %15.5e %15.5e\n',iter, NfunEvals/ndatapnts,S,f_s,f, Sample_Grad(c_stor,1),norm2grad(c_stor,1), norminfgrad(c_stor,1), alphak);
